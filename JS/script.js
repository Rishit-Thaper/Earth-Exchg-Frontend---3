...
//function to randomly change numbers
const phoneNumbers = [
  "918745997743",
  "919718228812",
  "918929955853",
...
function assignRandomPhoneNumber(buttonId) {
  const button = document.getElementById(buttonId);
  const randomIndex = Math.floor(Math.random() * phoneNumbers.length);
  const phoneNumber = phoneNumbers[randomIndex];
  button.href = `https://api.whatsapp.com/send?phone=${phoneNumber}`;
}

// Call the function to assign random phone numbers to each button
assignRandomPhoneNumber("button1");
assignRandomPhoneNumber("button2");
assignRandomPhoneNumber("button3");